...
1. **Secrets:** never commit `.env`/keys; never log/echo plaintext Shopify client secret, `shpat_` tokens, Meta WhatsApp token, or app secret; encrypted (Fernet) at rest in `app_config`; never return plaintext keys to any UI. Only `APP_MASTER_KEY` + `DATABASE_URL` live in env.
