...
- **Notes:** HMAC = base64(HMAC-SHA256(raw body, config `shopify:client_secret`)), constant-time. The atomic `IngestStore.ingest_order_created` transaction is followed by the ack (ADR-001; 5xx → Shopify retries), then two POST-ack side effects: the order-mirror sync (best-effort, non-fatal) and — **new 2026-08-13** — the **self-invoke send**: when `IngestResult.outbound_id is not None` (a fresh row was queued this call — not a duplicate, eligibility already passed) the handler registers `jobs.outbox_drain.send_outbound_now(c, outbound, outbound_id)` on FastAPI `BackgroundTasks`, which runs AFTER the 200 is constructed (within the request lifecycle Vercel's ASGI runtime waits for before freezing — safe on serverless, unlike bare fire-and-forget). This is the actual production send trigger — previously only the un-scheduled `run_outbox_drain` sent. It processes ONLY that one known row by id, NEVER `claim_queued_outbound` (that plain non-atomic `SELECT ... WHERE state='queued'` could let two concurrent orders both claim + double-send one row); honours the `send_mode` kill switch exactly like the drain (off → leave queued, shadow/allowlist-miss → suppress, no Meta call); can never raise (wrapped in `try/except`). Idempotent on `(webhook_id, topic)`. Eligibility from config `push_policy` (default `cod_only`) + `push_staleness_hours` (default 6, parsed via `_staleness_hours` = ASCII-digit gate + `math.isfinite` + range clamp → 6.0 so a corrupt/Unicode/non-finite stored value can't 500 the signed hot path — security-review 2026-08-06); queued push dedupe_key `order_created:{gid}`. Outbox payload_json: `{template:"order_confirmation_cod", language, customer_name, order_name, amount}`.
...
- **Request:** header `X-Cron-Secret`; path `{name}`. (Route is POST-only.)
- **Response:** 503 if `settings.cron_secret` unset/<16 chars; 403 on secret mismatch/missing/non-ASCII; 404 unknown job; 502 `{"job","error":"job failed"}` on a `ShopifyError` (text never echoed); 200 `{"job": name, "result": ...}`.
- **Notes:** constant-time secret compare (F11). Registered jobs: `ensure_subscription` (→ `{"status": "ok|created|updated"}` or `{"error": "public_base_url not configured"}`); `retention_purge` (DPDP age-based purge — reads config `retention_days`; default 0 → `{"status": "disabled"}` no-op; when >0 → `{"status": "purged", "retention_days", "deleted": {...}}`); **`outbox_drain` (Phase 5, 2026-08-10 — Flow A send drain, ADR-002 kill switch)** → `{"drained":0,...,"reason":"send_mode off"}` when off, `{"drained":0,"error":"whatsapp not configured"}` when unconfigured, else `{"drained","sent","suppressed","failed","undeliverable"}`; **`reconcile_cancels` (Phase 5, 2026-08-10 — apply the final `cancelled` tag once Shopify confirms cancelledAt)** → `{"checked","reconciled","pending","skipped"}`. Both are intended to be driven by a Vercel cron; both are idempotent + safe to re-run.
