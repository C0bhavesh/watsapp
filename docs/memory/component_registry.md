...
- **Used in:** Phase 4 conversation engine (Tasks 12/14 — customer_support agent handoff logic, webhook wiring); `core.memory` depends only on the `ConversationStore` Protocol (no concrete store import), per `core` layering rule.
