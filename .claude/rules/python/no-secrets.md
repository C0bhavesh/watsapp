...
- Covered secrets: Shopify client id/secret, `shpat_` access tokens, Meta WhatsApp token (`EAA…`), Meta app secret, webhook verify token, LLM keys. Only `APP_MASTER_KEY`, `DATABASE_URL` + `ADMIN_PASSWORD` live in env (`ADMIN_PASSWORD` owner-approved 2026-07-30).
