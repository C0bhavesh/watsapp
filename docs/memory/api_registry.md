...
- **Notes:** HMAC = **hex**(HMAC-SHA256(raw body, config `whatsapp:app_secret`)), constant-time (distinct from Shopify base64). Idempotent on Meta `message_id` via `MessageStore.record_if_new`. **Task 14 (Phase 4):** every fresh `InboundText` now runs the router + 5-agent pipeline and (gated) sends the reply, BEFORE the response is built, still inside the request (no queue) — a pipeline failure is swallowed by an internal `except Exception: logger.exception(...)` so the webhook still 200s the already-deduped message; a turn that runs past `TURN_TIMEOUT_SECONDS` (50s, comfortably under `vercel.json`'s `maxDuration=60`) is caught as `except TimeoutError` and logged as a WARNING instead of an uncaught platform 504 (2026-08-06 security-review fix — see below). Gates (see component_registry.md's `app/core/conversation.py` entry for the full order): `AdminControls.send_mode` `off` = pipeline does not run at all (no Shopify/LLM call); `shadow` = runs + persists but never `send_text`; `allowlist` = sends only to `controls.allowlist_phones`; `live` = always sends. `conversations.paused_until` (human took over) = the message IS still persisted via `append_message` but no router/agent/send runs. `InboundButton`/`InboundInteractive` are completely unaffected — still just recorded/echoed; their deterministic button-tap mutation dispatch is Phase 5, attaching at the same `event_type` seam. Never crashes on attacker-typed JSON. **Post-Task-14 security-review fixes (2026-08-06):** (1) **layering** — the pipeline logic moved out of `channels/whatsapp.py` into `app/core/conversation.py` (`fastapi-layering.md`; `channels/` is webhook edge only). (2) **`maxDuration`** added to `backend/vercel.json` (`functions["api/index.py"].maxDuration = 60`, alongside the existing `builds`/`routes` config) plus the internal `asyncio.timeout` above — without both, a worst-case turn (11 config reads + Fernet decrypts + Shopify fetches + two LLM calls + the WhatsApp send) risked an uncaught platform 504 whose retry would be silently dropped as a duplicate (message already marked seen), losing the reply with nothing logged. (3) **failed-send logging** — `SendResult.ok is False` is now checked and logged (`whatsapp send failed: status=%s error=%s`, `error` already secret-redacted) instead of silently discarded. (4) **empty-reply guard** — a reply that strips to blank (e.g. a completion that's only a code fence) falls back to `copy_for("error_fallback", "en")` before persist/send.
